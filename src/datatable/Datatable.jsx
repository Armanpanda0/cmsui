    ...
    paginatedData.map((row, rowIndex) => (
      <React.Fragment key={rowIndex}>
        <tr
          key={rowIndex}
          style={{
            backgroundColor: row.bgColor || (rowIndex % 2 === 0 ? '#ffffff' : '#f2f2f2'),
          }}
        >
        ...
        {/* Only show inner table if row.nestedData is defined */}
        {row.nestedData && (
          <tr>
            <td colSpan={columns.length}>
              <div className="inner-table-width">
                <table className="table table-bordered table-hover">
                  <thead>
                    <tr className="inner-sub-table-th">
                      {innerColumns.map((innerColumn, colIndex) => (
                        <th
                          className="inner-sub-table-th"
                          key={colIndex}
                          style={{ width: innerColumn.width }}
                        >
                          {innerColumn.name}
                        </th>
                      ))}
                  ...
                  <tbody>
                    {row.nestedData.map((nestedRow, nestedIndex) => (
                      <tr key={nestedIndex} className="dataTableBorderedRow">
                        {innerColumns.map((innerColumn, colIndex) => {
                          if (innerColumn.rowSpan && nestedRow.rowSpan > 0) {
                            return (
                              <td
                                ...
                                style={{ backgroundColor: innerColumn.bgColor || 'inherit' }}
                              >
                                {innerColumn.selector(nestedRow)}
                              </td>
                            );
                          } else if (
                            innerColumn.name === 'Parameter Name' ||
                            innerColumn.name === 'Parameter Value'
                          ) {
                            return (
                              <td
                                key={colIndex}
